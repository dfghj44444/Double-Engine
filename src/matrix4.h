...
namespace core
{
	...
	class CMatrix4
	{
		public:
			...
			//! Simple operator for linearly accessing every element of the matrix.
			const T& operator[](unsigned int index) const { return M[index]; }
		...
		private:
			...
			mutable unsigned int definitelyIdentityMatrix;
#endif
#if defined ( USE_MATRIX_TEST_DEBUG )
			unsigned int id;
	...
	*/
	...
	//! Transforms a axis aligned bounding box
	template <class T>
	inline void CMatrix4<T>::transformBox(core::aabbox3d<float>& box) const
	{
#if defined ( USE_MATRIX_TEST )
		if (isIdentity())
			return;
...
//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
	...
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildNDCToDCMatrix( const core::rect<int>& viewport, float zScale)
	{
	...
	*/
	...
	DLL_EXPORT extern const matrix4 IdentityMatrix;
