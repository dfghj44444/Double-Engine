...
#ifdef _COMPILE_WITH_WINDOWS_DEVICE_
...
namespace doubal
{
	namespace video
	{
		...
		IVideoDriver* createDirectX8Driver(const core::dimension2d<unsigned int>& screenSize, HWND window,
			unsigned int bits, bool fullscreen, bool stencilbuffer, io::IFileSystem* io,
			bool pureSoftware, bool highPrecisionFPU, bool vsync, char antiAlias);
		#endif

		#ifdef _IRR_COMPILE_WITH_DIRECT3D_9_
		IVideoDriver* createDirectX9Driver(const core::dimension2d<unsigned int>& screenSize, HWND window,
			unsigned int bits, bool fullscreen, bool stencilbuffer, io::IFileSystem* io,
			bool pureSoftware, bool highPrecisionFPU, bool vsync, char antiAlias);
		#endif
	}
...
doubal::CDeviceWin32* getDeviceFromHWnd(HWND hWnd)
{
	std::list<SEnvMapper>::iterator it = EnvMap.begin();
	for (; it!= EnvMap.end(); ++it)
		if ((*it).hWnd == hWnd)
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		{0, WM_LBUTTONDOWN, doubal::EMIE_LMOUSE_PRESSED_DOWN},
		{1, WM_LBUTTONUP,   doubal::EMIE_LMOUSE_LEFT_UP},
		{0, WM_RBUTTONDOWN, doubal::EMIE_RMOUSE_PRESSED_DOWN},
		{1, WM_RBUTTONUP,   doubal::EMIE_RMOUSE_LEFT_UP},
		{0, WM_MBUTTONDOWN, doubal::EMIE_MMOUSE_PRESSED_DOWN},
		{1, WM_MBUTTONUP,   doubal::EMIE_MMOUSE_LEFT_UP},
		{2, WM_MOUSEMOVE,   doubal::EMIE_MOUSE_MOVED},
		{3, WM_MOUSEWHEEL,  doubal::EMIE_MOUSE_WHEEL},
		{-1, 0, 0}
	};
...
//		event.EventType = doubal::EET_MOUSE_INPUT_EVENT;
//		event.MouseInput.Event = (doubal::EMOUSE_INPUT_EVENT) m->irrMessage;
...
//			event.MouseInput.ButtonStates |= doubal::EMBSM_MIDDLE;
//#if(_WIN32_WINNT >= 0x0500)
//		if (wParam & MK_XBUTTON1)
//			event.MouseInput.ButtonStates |= doubal::EMBSM_EXTRA1;
//		if (wParam & MK_XBUTTON2)
//			event.MouseInput.ButtonStates |= doubal::EMBSM_EXTRA2;
//#endif
...
//			event.MouseInput.Wheel = ((doubal::float)((short)HIWORD(wParam))) / (doubal::float)WHEEL_DELTA;
//		}
//
...
//
//			if ( event.MouseInput.Event >= doubal::EMIE_LMOUSE_PRESSED_DOWN && event.MouseInput.Event <= doubal::EMIE_MMOUSE_PRESSED_DOWN )
//			{
//				doubal::unsigned int clicks = dev->checkSuccessiveClicks(event.MouseInput.X, event.MouseInput.Y, event.MouseInput.Event);
//				if ( clicks == 2 )
//				{
//					event.MouseInput.Event = (doubal::EMOUSE_INPUT_EVENT)(doubal::EMIE_LMOUSE_DOUBLE_CLICK + event.MouseInput.Event-doubal::EMIE_LMOUSE_PRESSED_DOWN);
//					dev->postEventFromUser(event);
//				}
//				else if ( clicks == 3 )
//				{
//					event.MouseInput.Event = (doubal::EMOUSE_INPUT_EVENT)(doubal::EMIE_LMOUSE_TRIPLE_CLICK + event.MouseInput.Event-doubal::EMIE_LMOUSE_PRESSED_DOWN);
//					dev->postEventFromUser(event);
//				}
//			}
		//}
	...
	case WM_KEYUP:
		{
			...
			//event.EventType = doubal::EET_KEY_INPUT_EVENT;
			//event.KeyInput.Key = (doubal::EKEY_CODE)wParam;
			//event.KeyInput.PressedDown = (message==WM_KEYDOWN || message == WM_SYSKEYDOWN);

			//const UINT MY_MAPVK_VSC_TO_VK_EX = 3; // MAPVK_VSC_TO_VK_EX should be in SDK according to MSDN, but isn't in mine.
			//if ( event.KeyInput.Key == doubal::KEY_SHIFT )
			//{
			//	// this will fail on systems before windows NT/2000/XP, not sure _what_ will return there instead.
			//	event.KeyInput.Key = (doubal::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
			//}
			//if ( event.KeyInput.Key == doubal::KEY_CONTROL )
			//{
			//	event.KeyInput.Key = (doubal::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
			//	// some keyboards will just return LEFT for both - left and right keys. So also check extend bit.
			//	if (lParam & 0x1000000)
			//		event.KeyInput.Key = doubal::KEY_RCONTROL;
			//}
			//if ( event.KeyInput.Key == doubal::KEY_MENU )
			//{
			//	event.KeyInput.Key = (doubal::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
			//	if (lParam & 0x1000000)
			//		event.KeyInput.Key = doubal::KEY_RMENU;
			//}
	...
	case WM_USER:
		//event.EventType = doubal::EET_USER_EVENT;
		//event.UserEvent.UserData1 = (int)wParam;
		//event.UserEvent.UserData2 = (int)lParam;
		//dev = getDeviceFromHWnd(hWnd);
...
//! constructor
CDeviceWin32::CDeviceWin32(const SCreationParameters& params)
: CDeviceStub(params), HWnd(0), ChangedToFullScreen(false),
...
//! create the driver
void CDeviceWin32::createDriver()
{
	switch(CreationParams.DriverType)
	{
	case video::EDT_DIRECT3D9:
		#ifdef _COMPILE_WITH_DIRECT3D_9_
	...
	case video::EDT_BURNINGSVIDEO:
		#ifdef _COMPILE_WITH_BURNINGSVIDEO_
		switchToFullScreen();
...
void CDeviceWin32::yield()
{
	Sleep(1);
}

//! Pause execution and let other processes to run for a specified amount of time.
void CDeviceWin32::sleep(unsigned int timeMs, bool pauseTimer)
{
...
void CDeviceWin32::resizeIfNecessary()
{
	if (!Resized)
		return;
	...
	else
	{
		...
		getVideoDriver()->OnResize(doubal::core::dimension2du((unsigned int)r.right, (unsigned int)r.bottom));
...
bool CDeviceWin32::present(video::IImage* image, void* windowId, core::rect<int>* src)
{
	HWND hwnd = HWnd;
	if ( windowId )
	...
	if ( dc )
	{
		...
		bi.bV4Height        = -((int)image->getDimension().Height);
...
bool CDeviceWin32::isWindowActive() const
{
...
bool CDeviceWin32::isWindowFocused() const
{
...
bool CDeviceWin32::isWindowMinimized() const
{
	WINDOWPLACEMENT plc;
...
bool CDeviceWin32::switchToFullScreen(bool reset)
{
	if (!CreationParams.Fullscreen)
		return true;
	if (reset)
...
CDeviceWin32::CCursorControl* CDeviceWin32::getWin32CursorControl()
{
...
//! by the gfx adapter.
video::IVideoModeList* CDeviceWin32::getVideoModeList()
{
	if (!VideoModeList.getVideoModeCount())
	{
		...
		{
			VideoModeList.addMode(core::dimension2d<unsigned int>(mode.dmPelsWidth, mode.dmPelsHeight),
				mode.dmBitsPerPel);
		...
		if (EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &mode))
			VideoModeList.setDesktop(mode.dmBitsPerPel, core::dimension2d<unsigned int>(mode.dmPelsWidth, mode.dmPelsHeight));
	}
...
void CDeviceWin32::getWindowsVersion(core::stringc& out)
{
...
void CDeviceWin32::OnResized()
{
	Resized = true;
}

//! Sets if the window should be resizable in windowed mode.
void CDeviceWin32::setResizable(bool resize)
{
...
void CDeviceWin32::minimizeWindow()
{
...
void CDeviceWin32::maximizeWindow()
{
	WINDOWPLACEMENT wndpl;
	wndpl.length = sizeof(WINDOWPLACEMENT);
	GetWindowPlacement(HWnd, &wndpl);
...
void CDeviceWin32::restoreWindow()
{
	WINDOWPLACEMENT wndpl;
	wndpl.length = sizeof(WINDOWPLACEMENT);
	GetWindowPlacement(HWnd, &wndpl);
...
bool CDeviceWin32::activateJoysticks(core::array<SJoystickInfo> & joystickInfo)
{
#if defined _IRR_COMPILE_WITH_JOYSTICK_EVENTS_
	joystickInfo.clear();
	ActiveJoysticks.clear();

	const unsigned int numberOfJoysticks = ::joyGetNumDevs();
	JOYINFOEX info;
	...
	for(; joystick < numberOfJoysticks; ++joystick)
	{
											...
											sizeof(activeJoystick.Caps)))
		{
			...
			returnInfo.Joystick = (char)joystick;
...
void CDeviceWin32::pollJoysticks()
{
#if defined _IRR_COMPILE_WITH_JOYSTICK_EVENTS_
	...
	for(joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
	{
		...
		if(JOYERR_NOERROR == joyGetPosEx(ActiveJoysticks[joystick].Index, &info))
		{
			...
			event.EventType = doubal::EET_JOYSTICK_INPUT_EVENT;
			event.JoystickEvent.Joystick = (char)joystick;

			event.JoystickEvent.POV = (unsigned short)info.dwPOV;
...
bool CDeviceWin32::setGammaRamp( float red, float green, float blue, float brightness, float contrast )
{
	bool r;
...
bool CDeviceWin32::getGammaRamp( float &red, float &green, float &blue, float &brightness, float &contrast )
{
	bool r;
	unsigned short ramp[3][256];
...
void CDeviceWin32::clearSystemMessages()
{
	MSG msg;
...
void CDeviceWin32::ReportLastWinApiError()
{
