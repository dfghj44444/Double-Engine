...
namespace video
{
	...
	*/
	...
	public:
		...
		SColor (unsigned int a, unsigned int r, unsigned int g, unsigned int b)
		...
		SColor getInterpolated(const SColor &other, float d) const
		{
			...
			return SColor((unsigned int)core::round32(other.getAlpha()*inv + getAlpha()*d),
				(unsigned int)core::round32(other.getRed()*inv + getRed()*d),
				(unsigned int)core::round32(other.getGreen()*inv + getGreen()*d),
				(unsigned int)core::round32(other.getBlue()*inv + getBlue()*d));
		}
	...
	*/
	...
	public:
		...
		{
			return SColor((unsigned int)core::round32(a*255.0f), (unsigned int)core::round32(r*255.0f), (unsigned int)core::round32(g*255.0f), (unsigned int)core::round32(b*255.0f));
		}
	...
	*/
	...
	private:
		inline unsigned int toRGB1(float rm1, float rm2, float rh) const;

	};

	inline void SColorHSL::fromRGB(const SColor &color)
	{
		const unsigned int maxValInt = core::max_(color.getRed(), color.getGreen(), color.getBlue());
		const float maxVal = (float)maxValInt;
	...
	inline void SColorHSL::toRGB(SColor &color) const
	{
		...
		{
			unsigned char c = (unsigned char) ( Luminance * 255.0 );
			color.setRed(c);
			color.setGreen(c);
			color.setBlue(c);
			return;
		...
		const float rm1 = 2.0f * Luminance - rm2;
	...
	inline unsigned int SColorHSL::toRGB1(float rm1, float rm2, float rh) const
	{
		...
		return (unsigned int) core::round32(rm1 * 255.f);
	}
