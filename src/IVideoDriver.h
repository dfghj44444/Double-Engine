...
/* This file is part of the "Doubal Engine".
...
namespace video
{
	...
	were lost and need to be recreated. doubal will sometimes recreate the actual objects,
	...
	the doubal Engine: All rendering and texture manipulation is done with
	this interface. You are able to use the doubal Engine by only
	invoking methods of this interface if you like to, although the
	doubal::scene::ISceneManager interface provides a lot of powerful classes
	...
	*/
	...
	public:
		...
		*/
		...
		virtual unsigned int getImageLoaderCount() const = 0;
		...
		virtual IImageLoader* getImageLoader(unsigned int n) = 0;

		//! Retrieve the number of image writers
		/** \return Number of image writers */
		virtual unsigned int getImageWriterCount() const = 0;
		...
		virtual IImageWriter* getImageWriter(unsigned int n) = 0;
		...
		//virtual ITexture* getTextureByIndex(unsigned int index) =0;
		...
		information. */
		//virtual ITexture* addTexture(const core::dimension2d<unsigned int>& size,const io::path& name, ECOLOR_FORMAT format = ECF_A8R8G8B8) = 0;
		...
		IReferenceCounted::drop() for more information. */
		//virtual ITexture* addRenderTargetTexture(const core::dimension2d<unsigned int>& size,	const io::path& name = "rt", const ECOLOR_FORMAT format = ECF_UNKNOWN) =0;
		...
		(i.e. black). This behaviour matches the legacy (buggy) behaviour prior
		to release 1.5 and is provided for backwards compatibility only.*/
		//virtual void makeColorKeyTexture(video::ITexture* texture,core::position2d<int> colorKeyPixelPos,	bool zeroTexels = false) const =0;
		...
		ITexture* target = driver->addRenderTargetTexture(core::dimension2d<unsigned int>(128,128), "rtt1");
		...
		virtual bool setRenderTarget(const std::vector<video::IRenderTarget>& texture,
		...
		operations. */
		virtual void setViewPort(const core::rect<int>& area) =0;

		//! Gets the area of the current viewport.
		/** \return Rectangle of the current viewport. */
		virtual const core::rect<int>& getViewPort() const =0;
		...
		\param pType Primitive type, e.g. scene::EPT_TRIANGLE_FAN for a triangle fan.
		\param iType Index type, e.g. video::EIT_16BIT for 16bit indices. */
		virtual void draw2DVertexPrimitiveList(const void* vertices, unsigned int vertexCount,
				const void* indexList, unsigned int primCount,
				E_VERTEX_TYPE vType=EVT_STANDARD,
				scene::E_PRIMITIVE_TYPE pType=scene::EPT_TRIANGLES,
				E_INDEX_TYPE iType=EIT_16BIT) =0;
		...
		void drawIndexedTriangleList(const S3DVertex* vertices,
			unsigned int vertexCount, const unsigned short* indexList, unsigned int triangleCount)
		{
		...
		image will be drawn. */
		//virtual void draw2DImage(const video::ITexture* texture,const core::position2d<int>& destPos) =0;
...
#if 0
		virtual void draw2DImage(const video::ITexture* texture, const core::position2d<int>& destPos,
			const core::rect<int>& sourceRect, const core::rect<int>* clipRect =0,
		...
		virtual void draw2DImageBatch(const video::ITexture* texture,
				const core::position2d<int>& pos,
				const std::vector<core::rect<int> >& sourceRects,
				const std::vector<int>& indices,
				int kerningWidth=0,
				const core::rect<int>* clipRect=0,
...
#if 0
		virtual void draw2DImageBatch(const video::ITexture* texture,
				const std::vector<core::position2d<int> >& positions,
				const std::vector<core::rect<int> >& sourceRects,
				const core::rect<int>* clipRect=0,
				SColor color=SColor(255,255,255,255),
				bool useAlphaChannelOfTexture=false) =0;
		...
		blended. */
#if 0
		virtual void draw2DImage(const video::ITexture* texture, const core::rect<int>& destRect,
			const core::rect<int>& sourceRect, const core::rect<int>* clipRect =0,
		...
		performed. */
		virtual void draw2DRectangle(SColor color, const core::rect<int>& pos,
			const core::rect<int>* clip =0) =0;
		...
		will be clipped. If the pointer is null, no clipping will be
		performed. */
		virtual void draw2DRectangle(const core::rect<int>& pos,
		...
		virtual void draw2DLine(const core::position2d<int>& start,
					const core::position2d<int>& end,
					SColor color=SColor(255,255,255,255)) =0;
		...
		virtual void drawPixel(unsigned int x, unsigned int y, const SColor& color) =0;
		...
		virtual void draw2DPolygon(core::position2d<int> center,
				float radius,
				video::SColor color=SColor(100,255,255,255),
				int vertexCount=10) =0;
		...
		zpass. */
		virtual void drawStencilShadowVolume(const core::vector3df* triangles, int count, bool zfail=true) =0;
		...
		virtual const core::dimension2d<unsigned int>& getScreenSize() const =0;
		...
		virtual const core::dimension2d<unsigned int>& getCurrentRenderTargetSize() const =0;
		...
		virtual unsigned int getPrimitiveCountDrawn( unsigned int mode =0 ) const =0;
		...
		virtual int addDynamicLight(const SLight& light) =0;
		...
		virtual unsigned int getDynamicLightCount() const =0;
		...
		\return Light data. */
		virtual const SLight& getDynamicLight(unsigned int idx) const =0;

		//! Turns a dynamic light on or off
		//! \param lightIndex: the index returned by addDynamicLight
		//! \param turnOn: true to turn the light on, false to turn it off
		virtual void turnLightOn(int lightIndex, bool turnOn) =0;
		...
		/** This is useful if the doubal Engine should be able to load
		...
		/** This is useful if the doubal Engine should be able to
		...
		virtual unsigned int getMaximalPrimitiveCount() const =0;
		...
		//virtual bool writeImageToFile(IImage* image, const io::path& filename, unsigned int param = 0) = 0;
		...
		level).
		\return True on successful write. */
		//virtual bool writeImageToFile(IImage* image, io::IWriteFile* file, unsigned int param =0) =0;
		...
		virtual IImage* createImageFromData(ECOLOR_FORMAT format,
			const core::dimension2d<unsigned int>& size, void *data,
		...
		If you no longer need the image, you should call IImage::drop().
		See IReferenceCounted::drop() for more information. */
		virtual IImage* createImage(ECOLOR_FORMAT format, const core::dimension2d<unsigned int>& size) =0;
		...
		virtual IImage* createImage(IImage* imageToCopy,
				const core::position2d<int>& pos,
				const core::dimension2d<unsigned int>& size) =0;
		...
		virtual void OnResize(const core::dimension2d<unsigned int>& size) =0;
		...
		//virtual int addMaterialRenderer(IMaterialRenderer* renderer, const char* name =0) =0;
		...
		//virtual IMaterialRenderer* getMaterialRenderer(unsigned int idx) =0;

		//! Get amount of currently available material renderers.
		/** \return Amount of currently available material renderers. */
		virtual unsigned int getMaterialRendererCount() const =0;
		...
		exisiting */
		virtual const char* getMaterialRendererName(unsigned int idx) const =0;
		...
		virtual void setMaterialRendererName(int idx, const char* name) =0;
		...
		*/
		...
		virtual bool setClipPlane(unsigned int index, const core::plane3df& plane, bool enable=false) =0;
		...
		\param enable If true, enable the clipping plane else disable
		it. */
		virtual void enableClipPlane(unsigned int index, bool enable) =0;

		//! Set the minimum number of vertices for which a hw buffer will be created
		/** \param count Number of vertices to set as minimum. */
		virtual void setMinHardwareBufferVertexCount(unsigned int count) =0;
