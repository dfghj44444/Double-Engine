...
// For conditions of distribution and use, see copyright notice in double.h
#ifndef __I_DEVICE_H_
#define __I_DEVICE_H_
...
namespace doubal
{
	...
	//! The double device. You can create it with createDevice() or createDeviceEx().
	/** This is the most important class of the double Engine. You can
	...
	*/
	...
	public:
		...
		calling doubleDevice::run(). Returns false if device wants
		...
		Note if you are running double inside an external, custom
		created window: Calling Device->run() will cause double to
		dispatch windows messages internally.
		If you are running double in your own custom window, you can
		also simply use your own message loop using GetMessage,
		DispatchMessage and whatever and simply don't use this method.
		But note that double will not be able to fetch user input
		then. See double::SdoubleCreationParameters::WindowId for more
		...
		*/
		virtual void sleep(unsigned int timeMs, bool pauseTimer=false) = 0;
		...
		create an double Device with a video mode before being able
		to get the video mode list, let me tell you that there is no
		need to start up an double Device with EDT_DIRECT3D8,
		...
		//! Checks if the double window has focus
		/** \return True if window has focus. */
		virtual bool isWindowFocused() const = 0;

		//! Checks if the double window is minimized
		/** \return True if window is minimized. */
		virtual bool isWindowMinimized() const = 0;

		//! Checks if the double window is running in fullscreen mode
		...
		/** doubleDevice::run() will always return false after closeDevice() was called. */
		...
		on its type. See double::EEVENT_TYPE for details.
		...
		/** double contains support for joysticks, but does not generate joystick events by default,
		as this would consume joystick info that 3rd party libraries might rely on. Call this method to
		activate joystick support in double and to receive double::SJoystickEvent events.
		\param joystickInfo On return, this will contain an array of each joystick that was found and activated.
		\return true if joysticks are supported on this device and _double_COMPILE_WITH_JOYSTICK_EVENTS_
...
#ifdef _double_COMPILE_WITH_SOFTWARE_
					return true;
#else
					return false;
#endif
				case video::EDT_BURNINGSVIDEO:
#ifdef _double_COMPILE_WITH_BURNINGSVIDEO_
					return true;
#else
					return false;
#endif
				case video::EDT_DIRECT3D9:
#ifdef _double_COMPILE_WITH_DIRECT3D_9_
					return true;
#else
					return false;
#endif
